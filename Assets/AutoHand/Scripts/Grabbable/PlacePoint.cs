...
if (resizeOnPlace) {
    if (placedObject != null) {
        // Remove Events
        if (placedObject.OnBeforeGrabEvent != null) {
            placedObject.OnBeforeGrabEvent -= ResizeBeforeGrab;
        }

        if (placedObject.grabbableChildren != null && placedObject.grabbableChildren.Count > 0) {
            foreach (var grabbable in placedObject.grabbableChildren) {
                if (grabbable != null && grabbable.OnBeforeGrabEvent != null) {
                    grabbable.OnBeforeGrabEvent -= ResizeBeforeGrab;
                }
            }
        }
    } else {
        Debug.LogWarning("placedObject es nulo. No se pueden eliminar eventos.");
    }

    // If the remove function is called without grabbing
    if (placeObj != null && placeObj.HeldCount() == 0) {
        if (placeObj.rootTransform != null) {
            placeObj.rootTransform.localScale = prefitScale;
        } else {
            Debug.LogWarning("rootTransform de placeObj es nulo.");
        }
    } else {
        Debug.LogWarning("placeObj es nulo o HeldCount no es igual a 0.");
    }

    if (placeObj != null) {
        placeObj.instantGrab = wasInstantGrab;
    } else {
        Debug.LogWarning("placeObj es nulo al intentar restaurar instantGrab.");
    }
}
    ...
    if (from == null) {
        Debug.LogWarning("Attempted to highlight a null Grabbable.");
        return;
    }
